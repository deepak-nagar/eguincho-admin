...
<template>
  ...
  >
    <div class="mt-6 flex items-center justify-between px-6">
      <h4>
        {{ Object.entries(this.data).length === 0 ? "ADD NEW" : "UPDATE" }} CAR
      </h4>
      <feather-icon
        icon="XIcon"
      ...
      <div class="p-6">
        ...
        <div class="upload-img mt-5" v-if="!dataImg">
          ...
          <vs-button @click="$refs.uploadImgInput.click()"
            >Select an image for the car</vs-button
          >
        </div>
      </div>
    </component>
...
export default {
  ...
  watch: {
    isSidebarActive(val) {
      ...
      if (Object.entries(this.data).length === 0) {
        ...
        const { id, img, name, order_status, desc } = JSON.parse(
  ...
  computed: {
    ...
    isFormValid() {
      return !this.errors.any() && this.dataName && this.dataImg;
    },
    scrollbarTag() {
  ...
  methods: {
    initValues() {
      ...
      this.dataName = "";
      this.dataDesc = "";
      this.dataOrder_status = "available";
